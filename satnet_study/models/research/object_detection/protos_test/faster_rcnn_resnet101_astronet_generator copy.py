...
class AstroNetFasterRcnnResnet101Generator:
    def __init__(self,
        ...
        self.faster_rcnn_astronet.model.CopyFrom(detection_model)
        self.faster_rcnn_astronet.train_config.CopyFrom(train_config)
        self.faster_rcnn_astronet.eval_config.CopyFrom(eval_config)
        self.faster_rcnn_astronet.train_input_reader.CopyFrom(train_input_reader)
        self.faster_rcnn_astronet.eval_input_reader.CopyFrom(eval_input_reader)
...
def main(unused_argv):
    ...
    detection_model = DetectionModel(faster_rcnn.faster_rcnn)
    train_config = TrainConfig()
    eval_config = EvalConfig()

    train_tfrecord_input_reader = TrainTFRecordInputReader()
    eval_tfrecord_input_reader = EvalTFRecordInputReader()

    train_input_reader = TrainInputReader(train_tfrecord_input_reader.tf_record_input_reader)
    eval_input_reader = EvalInputReader(eval_tfrecord_input_reader.tf_record_input_reader)

    config_generator = AstroNetFasterRcnnResnet101Generator(config_file,
                                                            detection_model.detection_model,
                                                            train_config.train_config,
                                                            eval_config.eval_config,
                                                            train_input_reader.input_reader,
                                                            eval_input_reader.input_reader)
    config_generator.batch_size(16)
    config_generator.initial_learning_rate(0.001)
    config_generator.output_config()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config_file',
                        type=str, 
                        default='faster_rcnn_resnet101_astronet_test.config', 
